...
const UnEnrollButton = ({ id }: { id: string }) => {
  ...
  return (
    session.data?.user.privilege === 'STUDENT' && (
      ...
      disabled={isLoading}
        ...
        className="rounded-full disabled:opacity-80 bg-red-700 opacity-95 hover:opacity-100 text-white text-sm font-medium capitalize py-1 grid place-items-center gap-2 px-4">
        <span>unenroll</span>
      </button>
    )
  );
